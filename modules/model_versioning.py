"""
Model versioning module for MH-Net.

This module provides functionality for model versioning, tracking,
...
# Conditionally import TensorFlow to handle environments where it may not be available
try:
    import tensorflow as tf
    TENSORFLOW_AVAILABLE = True
except ImportError:
    # Import our mock implementation
    from modules.mock_tensorflow import tf
    TENSORFLOW_AVAILABLE = False
    print("TensorFlow not available. Using mock implementation for compatibility.")
...
class ModelRegistry:
            ...
            if os.path.isfile(model_file):
                ...
                if model.model_type == "tensorflow":
                    ...
                    if not TENSORFLOW_AVAILABLE:
                        raise ValueError("Cannot save TensorFlow model: TensorFlow not available")
                        
                    try:
                        # Check if it's a saved model or h5 file
                        if model_file.endswith('.h5') or model_file.endswith('.hdf5'):
                            # Load and resave as SavedModel
                            tf_model = tf.keras.models.load_model(model_file)
                            tf_model.save(model_path)
                        else:
                            # Copy directory
                            shutil.copytree(model_file, model_path, dirs_exist_ok=True)
                    except Exception as e:
                        raise ValueError(f"Failed to save TensorFlow model: {e}")
                else:
                    # Copy file
    ...
    def load_model(self, name: str, version: Optional[str] = None):
        ...
        if not model_info:
            raise ValueError(f"Model not found: {name}/{version if version else 'latest'}")
        
        # Get model path
        model_dir = os.path.join(self.models_dir, model_info["name"], model_info["version"])
        
        # Load based on model type
        if model_info["model_type"] == "tensorflow":
            # Check if TensorFlow is available
            if not TENSORFLOW_AVAILABLE:
                print(f"Warning: Cannot load TensorFlow model {name}/{version}. TensorFlow is not available.")
                return None
...
class ModelDeployment:
    ...
    def get_deployments(self) -> List[Dict[str, Any]]:
        ...
        for name, deployment in self.deployed_models.items():
            # Create deployment info without the model object
            deployment_info = {
                "name": name,
    ...
    def predict(self, deployment_name: str, inputs: Any) -> Dict[str, Any]:
        ...
        if deployment["status"] != "active":
            raise ValueError(f"Deployment is not active: {deployment_name}")
        ...
        try:
            ...
            # TensorFlow model handling - we now always have a tf object (either real or mock)
            if model and (hasattr(tf.keras, 'Model') and isinstance(model, tf.keras.Model)):
                ...
                try:
                    if not isinstance(inputs, np.ndarray):
                        try:
                            ...
                            # If numpy conversion fails, just use the inputs as-is
                            pass
                    ...
                    if isinstance(raw_predictions, list):
                        predictions = [p.tolist() if hasattr(p, 'tolist') else p for p in raw_predictions]
                    elif hasattr(raw_predictions, 'tolist'):
                        predictions = raw_predictions.tolist()
                    else:
                        predictions = raw_predictions
                except Exception as e:
                    # If prediction fails, provide placeholder prediction
                    predictions = [0.5]  # Placeholder prediction
                    print(f"Warning: Prediction failed: {e}. Using placeholder prediction.")
            elif model is None:
                # Handle the case when the model couldn't be loaded
                predictions = [0.5]  # Placeholder prediction
                print("Warning: Model not available, returning placeholder prediction")
            else:
                # For other model types
